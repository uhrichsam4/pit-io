...
for (const name of presets) {
  const shot = await withDev(async () => {
    await page.evaluate(() => {
      // MATSHOT ONLY: other agents are mid-edit in this tree; a module whose
      // per-frame hook throws would abort the render before any pixels exist.
      const g = window.DEV.game;
      for (const k of ['pedestrianUpdate', 'trafficUpdate']) {
        const f = g[k];
        if (!f) continue;
        g[k] = (dt) => { try { f(dt); } catch (e) { g[k] = null; } };
      }
    });
    await page.evaluate(
    ...
    // Five agents are saving into this tree; a Vite reload landing between the
    // preset and the shutter yields a picture of the loading screen. A frame
    // with almost no draw calls is that, not a render — throw so withDev
    // replays the whole shot.
    if (!stats || stats.drawCalls < 150) {
      throw new Error(`Execution context was destroyed: only ${stats && stats.drawCalls} draw calls`);
    }
